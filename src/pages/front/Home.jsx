...
export default function Home() {
    ...
    const getCartData=async()=>{
        try {
          const response = await fetch(`${process.env.REACT_APP_BACKEND_API_URL}/api/search?q=${search?.query}`, {
                    ...
                    <div className="row g-5 align-items-center">
                        <div className="col-md-12 col-lg-7">
                            ...
                            <div className="position-relative mx-auto">
                                <input className="form-control border-2 border-secondary w-75 py-3 px-4 rounded-pill"  placeholder="Search" onChange={(e)=>setSearch((x)=>({...x,query:e?.target?.value}))} />
                                <button type="submit" className="btn btn-primary border-2 border-secondary py-3 px-4 position-absolute rounded-pill text-white h-100" style={{
                                    top: 0, right: "25%"
                                }} onClick={getCartData}>Submit Now</button>
                            </div>
                        </div>
                            ...
                            <div id="tab-1" className="tab-pane fade show p-0 active">
                                <div className="row g-4">
                                    <div className="col-lg-12">
                                        <div className="row g-4">
                                            {
                                                search?.data.length===0 && products.length > 0 && products.map((product, index) => (
                                                    <ProductItem product={product}  key={index} size="3"/>
                                                ))
                                            }
                                             {
                                                search?.data.length > 0 && search?.data.map((product, index) => (
                                                    <ProductItem product={product}  key={index} size="3"/>
                                                ))
                                            }
                                        </div>
